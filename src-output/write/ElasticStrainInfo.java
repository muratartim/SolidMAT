...
/**
 * Class for writing elastic strains information to output file.
 * 
 * @author Murat
 * 
 */
public class ElasticStrainInfo extends Writer {
	...
	/**
	 * Creates ElasticStrainInfo object.
	 * 
	 * @param stations
	 *            The number of output stations.
	 */
	public ElasticStrainInfo(int stations) {
		stations_ = stations;
	}

	/**
	 * Writes elastic strains information to output file.
	 * 
	 * @param structure
	...
	private void writeTable1() {
		...
		try {
			...
			bwriter_
					.write(header("Elastic Strains (One dimensional elements), 1"));
			...
			// write headers
			String[] table = { "Element", "Station", "e11", "e22", "e33" };
			...
			for (int i = 0; i < element1D_.size(); i++) {
				...
				for (int j = 0; j < stations_; j++) {
					...
					// write components
					table[2] = formatter(strain.get(0, 0));
					table[3] = formatter(strain.get(1, 1));
					table[4] = formatter(strain.get(2, 2));
	...
	private void writeTable2() {
		...
		try {
			...
			// write header for first part of table
			bwriter_
					.write(header("Elastic Strains (One dimensional elements), 2"));
			...
			// write headers
			String[] table = { "Element", "Station", "e12", "e13", "e23" };
			bwriter_.write(table(table));
			...
			for (int i = 0; i < element1D_.size(); i++) {
				...
				for (int j = 0; j < stations_; j++) {
					...
					// write components
					table[2] = formatter(strain.get(0, 1));
					table[3] = formatter(strain.get(0, 2));
					table[4] = formatter(strain.get(1, 2));
	...
	private void writeTable3() {
		...
		try {
			...
			// write header for first part of table
			bwriter_
					.write(header("Elastic Strains (Two dimensional elements), 1"));
			...
			// write headers
			String[] table = { "Element", "Corner", "e11", "e22", "e33" };
			bwriter_.write(table(table));
	...
	private void writeTable4() {
		...
		try {
			...
			// write header for first part of table
			bwriter_
					.write(header("Elastic Strains (Two dimensional elements), 2"));
			...
			// write headers
			String[] table = { "Element", "Corner", "e12", "e13", "e23" };
			bwriter_.write(table(table));
	...
	/**
	 * Writes table for three dimensional elements part 1.
	 * 
	 */
	private void writeTable5() {
		...
		try {
			...
			// write header for first part of table
			bwriter_
					.write(header("Elastic Strains (Three dimensional elements), 1"));
			...
			// write headers
			String[] table = { "Element", "Vertex", "e11", "e22", "e33" };
			bwriter_.write(table(table));
	...
	/**
	 * Writes table for three dimensional elements part 2.
	 * 
	 */
	private void writeTable6() {
		...
		try {
			...
			// write header for first part of table
			bwriter_
					.write(header("Elastic Strains (Three dimensional elements), 2"));
			...
			// write headers
			String[] table = { "Element", "Vertex", "e12", "e13", "e23" };
			bwriter_.write(table(table));
			...
			for (int i = 0; i < element3D_.size(); i++) {
				...
				// compute vertex points in natural coordinates
				double[] eps1 = null;
				double[] eps2 = null;
				double[] eps3 = null;
				if (e.getGeometry() == Element3D.hexahedral_) {
