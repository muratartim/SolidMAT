...
/**
 * Class for writing initial displacement information to output file.
 * 
 * @author Murat
 * 
 */
public class InitialDispInfo extends Writer {

	/** Buffered writer. */
	private BufferedWriter bwriter_;

	/**
	 * Writes initial displacement information to output file.
	 * 
	 * @param structure
	...
	private void writeTable1(Structure s) {
		...
		try {
			...
			bwriter_.write(header("Initial displacements, 1"));
			...
			for (int i = 0; i < s.getNumberOfNodes(); i++) {
				...
				// get initial displacements of node
				Vector<InitialDisp> dl = n.getAllInitialDisp();

				// check if any available
				if (dl != null) {

					// loop over initial displacements of node
					for (int j = 0; j < dl.size(); j++) {

						// get initial displacement
						InitialDisp load = dl.get(j);

						// get properties
						table[0] = Integer.toString(i);
						table[1] = load.getName();
						table[2] = load.getBoundaryCase().getName();
						if (load.getCoordinateSystem() == InitialDisp.global_)
							table[3] = "Global";
						else
							table[3] = "Local";
	...
	private void writeTable2(Structure s) {
		...
		try {
			...
			// write header for first part of table
			bwriter_.write(header("Initial displacements, 2"));
	...
	private void writeTable3(Structure s) {
		...
		try {
			...
			// write header for first part of table
			bwriter_.write(header("Initial displacements, 3"));
