...
/**
 * Class for Element21. Properties of element are; Mechanics: Sanders'
 * doubly-curved shell, Geometric: Linear, Material: Orthotropic/Isotropic,
 * Interpolation degree: Biquadratic, Interpolation family: Lagrange, Number of
 * nodes: 6, Geometry: Triangular, Dofs (per node): ux, uy, uz, rx, ry, rz
 ...
 * 
 */
public class Element21 extends Element2D {
	 ...
	 */
	public Element21(Node n1, Node n2, Node n3, Node n4, Node n5, Node n6) {
		nodes_[0] = n1;
		nodes_[1] = n2;
		nodes_[2] = n3;
		nodes_[3] = n4;
	...
	private DMat getUncondensedStiffness() {
		...
		for (int i = 0; i < k1.rowCount(); i++) {
			for (int j = 0; j < k1.columnCount(); j++) {
				...
				kLocal.set(i + 12, j + 30, zeta * k4.get(i, j));
				kLocal.set(i + 12, j + 36, chi * k4.get(i, j));
				kLocal.set(i + 12, j + 48, k2.get(i, j));
				kLocal.set(i + 12, j + 60, phi * k1.get(i, j));
				kLocal.set(i + 18, j + 18, psi * k1.get(i, j));
				kLocal.set(i + 18, j + 30, omega * k4.get(i, j));
				kLocal.set(i + 18, j + 36, oo * k4.get(i, j));
				kLocal.set(i + 18, j + 54, k3.get(i, j));
				kLocal.set(i + 18, j + 60, nu * k1.get(i, j));
