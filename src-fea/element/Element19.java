...
/**
 * Class for Element19. Properties of element are; Mechanics: Sanders'
 * doubly-curved shell, Geometric: Linear, Material: Orthotropic/Isotropic,
 * Interpolation degree: Bilinear, Interpolation family: Lagrange, Number of
 * nodes: 4, Geometry: Quadrilateral, Dofs (per node): ux, uy, uz, rx, ry, rz
 ...
 * 
 */
public class Element19 extends Element2D {
	...
	/**
	 * Creates Element19 element. Nodes should be given in counter-clockwise
	 * order.
	 * 
	 * @param n1
	 ...
	 */
	public Element19(Node n1, Node n2, Node n3, Node n4) {
		nodes_[0] = n1;
		nodes_[1] = n2;
		nodes_[2] = n3;
		nodes_[3] = n4;
	...
	protected Interpolation2D getInterpolation() {
		...
		int degree = Interpolation2D.bilinear_;
	...
	protected DMat computeStiffnessMatrix() {
		...
		kLocal = kLocal.condense(20);

		// node-wise sorting of element stiffness matrix
		// sort by columns
		DMat kLocal1 = new DMat(20, 20);
		for (int i = 0; i < 20; i++)
			for (int j = 0; j < 5; j++)
				for (int k = 0; k < 4; k++)
					kLocal1.set(i, j + 5 * k, kLocal.get(i, 4 * j + k));

		// sort by rows
		DMat kLocal2 = new DMat(20, 20);
		for (int i = 0; i < 5; i++)
			for (int j = 0; j < 20; j++)
				for (int k = 0; k < 4; k++)
					kLocal2.set(i + 5 * k, j, kLocal1.get(4 * i + k, j));
	...
	protected DMat computeMassMatrix() {
		...
		DMat mLocal = new DMat(20, 20);
		for (int i = 0; i < k1.rowCount(); i++) {
			for (int j = 0; j < k1.columnCount(); j++) {
				mLocal.set(i, j, i0 * k1.get(i, j));
				mLocal.set(i + 4, j + 4, i0 * k1.get(i, j));
				mLocal.set(i + 8, j + 8, i0 * k1.get(i, j));
				mLocal.set(i + 12, j + 12, i2 * k1.get(i, j));
				mLocal.set(i + 16, j + 16, i2 * k1.get(i, j));
			}
		}

		// node-wise sorting of element mass matrix
		// sort by columns
		DMat mLocal1 = new DMat(20, 20);
		for (int i = 0; i < 20; i++)
			for (int j = 0; j < 5; j++)
				for (int k = 0; k < 4; k++)
					mLocal1.set(i, j + 5 * k, mLocal.get(i, 4 * j + k));

		// sort by rows
		DMat mLocal2 = new DMat(20, 20);
		for (int i = 0; i < 5; i++)
			for (int j = 0; j < 20; j++)
				for (int k = 0; k < 4; k++)
					mLocal2.set(i + 5 * k, j, mLocal1.get(4 * i + k, j));
	...
	protected DVec computeTempLoadVector() {
		...
		DVec tLoad = new DVec(32);
		for (int i = 0; i < r.rowCount(); i++) {
			tLoad.set(i + 8, etha * r.get(i, 0));
			tLoad.set(i + 12, nu * r.get(i, 0));
		}

		// get uncondensed element stiffness matrix
		DMat ke = getUncondensedStiffness();
		DMat krrInv = ke.getSubMatrix(0, 0, 31, 31).invert();
		DMat kruTr = ke.getSubMatrix(0, 32, 31, 51).transpose();
		...
		DVec tLoad1 = new DVec(20);
		for (int i = 0; i < 5; i++)
			for (int j = 0; j < 4; j++)
				tLoad1.set(i + 5 * j, tLoad.get(4 * i + j));
	...
	private DMat getUncondensedStiffness() {
		...
		DMat k3 = new DMat(4, 4);
		k3 = getSubMatrix(2, k3);

		// compute sub-matrix k4 (under-integrated)
		DMat k4 = new DMat(4, 4);
		k4 = getSubMatrix(3, k4);

		// compute sub-matrix k5 (under-integrated)
		DMat k5 = new DMat(4, 4);
		k5 = getSubMatrix(4, k5);

		// compute sub-matrix k6 (under-integrated)
		DMat k6 = new DMat(4, 4);
		k6 = getSubMatrix(5, k6);

		// insert sub-matrices to stiffness matrix
		DMat kLocal = new DMat(52, 52);
		for (int i = 0; i < k1.rowCount(); i++) {
			for (int j = 0; j < k1.columnCount(); j++) {
				kLocal.set(i, j, lambda * k1.get(i, j));
				kLocal.set(i, j + 32, -phi * k1.get(i, j));
				kLocal.set(i, j + 40, k5.get(i, j));
				kLocal.set(i, j + 48, k4.get(i, j));
				kLocal.set(i + 4, j + 4, beta * k1.get(i, j));
				kLocal.set(i + 4, j + 36, -nu * k1.get(i, j));
				kLocal.set(i + 4, j + 40, k6.get(i, j));
				kLocal.set(i + 4, j + 44, -k4.get(i, j));
				kLocal.set(i + 8, j + 8, gamma * k1.get(i, j));
				kLocal.set(i + 8, j + 12, alpha * k1.get(i, j));
				kLocal.set(i + 8, j + 20, zeta * k4.get(i, j));
				kLocal.set(i + 8, j + 24, chi * k4.get(i, j));
				kLocal.set(i + 8, j + 32, k2.get(i, j));
				kLocal.set(i + 8, j + 40, phi * k1.get(i, j));
				kLocal.set(i + 12, j + 12, psi * k1.get(i, j));
				kLocal.set(i + 12, j + 20, omega * k4.get(i, j));
				kLocal.set(i + 12, j + 24, oo * k4.get(i, j));
				kLocal.set(i + 12, j + 36, k3.get(i, j));
				kLocal.set(i + 12, j + 40, nu * k1.get(i, j));
				kLocal.set(i + 16, j + 16, theta * k1.get(i, j));
				kLocal.set(i + 16, j + 32, k3.get(i, j));
				kLocal.set(i + 16, j + 36, k2.get(i, j));
				kLocal.set(i + 20, j + 20, curlPhi * k1.get(i, j));
				kLocal.set(i + 20, j + 24, capLamb * k1.get(i, j));
				kLocal.set(i + 20, j + 48, k2.get(i, j));
				kLocal.set(i + 24, j + 24, mu * k1.get(i, j));
				kLocal.set(i + 24, j + 44, -k3.get(i, j));
				kLocal.set(i + 28, j + 28, kappa * k1.get(i, j));
				kLocal.set(i + 28, j + 44, -k2.get(i, j));
				kLocal.set(i + 28, j + 48, k3.get(i, j));
			}
		}
