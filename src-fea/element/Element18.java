...
/**
 * Class for Element18. Properties of element are; Mechanics: Mindlin/Reissner
 * shell, Geometric: Linear, Material: Orthotropic/Isotropic, Interpolation
 * degree: Biquadratic, Interpolation family: Lagrange, Number of nodes: 6,
 * Geometry: Triangular, Dofs (per node): ux, uy, uz, rx, ry, rz (global), u1,
 * u2, u3, r1, r2 (local), Mechanical loading: fx, fy, fz, mx, my, mz (global),
 * f1, f2, f3, m1, m2 (local), Temperature loading: yes, Displacements: u1, u2,
 * u3, r1, r2, Strains: e11, e22, e12, e13, e23 Stresses: s11, s22, s33, s12,
 * s13, s23, Internal forces: P11, N22, Q12, F13, H23, K22, M11, T12. Note:
 * Transverse shear terms are under-integrated against shear locking.
 * 
 * @author Murat
 * 
 */
public class Element18 extends Element2D {
	 ...
	 */
	public Element18(Node n1, Node n2, Node n3, Node n4, Node n5, Node n6) {
		nodes_[0] = n1;
		nodes_[1] = n2;
		nodes_[2] = n3;
		nodes_[3] = n4;
	...
	public DMat getStrain(double eps1, double eps2, double eps3) {
		...
		double u11 = 0.0, u12 = 0.0, u21 = 0.0, u22 = 0.0, u31 = 0.0, u32 = 0.0;
	...
	protected DMat computeStiffnessMatrix() {
		...
		kLocal = kLocal.condense(30);

		// node-wise sorting of element stiffness matrix
		// sort by columns
		DMat kLocal1 = new DMat(30, 30);
		for (int i = 0; i < 30; i++)
			for (int j = 0; j < 5; j++)
				for (int k = 0; k < 6; k++)
					kLocal1.set(i, j + 5 * k, kLocal.get(i, 6 * j + k));

		// sort by rows
		DMat kLocal2 = new DMat(30, 30);
		for (int i = 0; i < 5; i++)
			for (int j = 0; j < 30; j++)
				for (int k = 0; k < 6; k++)
					kLocal2.set(i + 5 * k, j, kLocal1.get(6 * i + k, j));
	...
	protected DMat computeMassMatrix() {
		...
		for (int i = 0; i < k1.rowCount(); i++) {
			for (int j = 0; j < k1.columnCount(); j++) {
				...
				mLocal.set(i + 6, j + 6, i0 * k1.get(i, j));
				mLocal.set(i + 12, j + 12, i0 * k1.get(i, j));
				mLocal.set(i + 18, j + 18, i2 * k1.get(i, j));
		...
		// sort by columns
		DMat mLocal1 = new DMat(30, 30);
		for (int i = 0; i < 30; i++)
			for (int j = 0; j < 5; j++)
				for (int k = 0; k < 6; k++)
					mLocal1.set(i, j + 5 * k, mLocal.get(i, 6 * j + k));

		// sort by rows
		DMat mLocal2 = new DMat(30, 30);
		for (int i = 0; i < 5; i++)
			for (int j = 0; j < 30; j++)
				for (int k = 0; k < 6; k++)
					mLocal2.set(i + 5 * k, j, mLocal1.get(6 * i + k, j));
	...
	protected DVec computeTempLoadVector() {
		...
		DVec tLoad = new DVec(48);
		for (int i = 0; i < r.rowCount(); i++) {
			tLoad.set(i + 12, etha * r.get(i, 0));
			tLoad.set(i + 18, nu * r.get(i, 0));
		}

		// get uncondensed element stiffness matrix
		DMat ke = getUncondensedStiffness();
		DMat krrInv = ke.getSubMatrix(0, 0, 47, 47).invert();
		DMat kruTr = ke.getSubMatrix(0, 48, 47, 77).transpose();
		...
		DVec tLoad1 = new DVec(30);
		for (int i = 0; i < 5; i++)
			for (int j = 0; j < 6; j++)
				tLoad1.set(i + 5 * j, tLoad.get(6 * i + j));
	...
	private DMat getUncondensedStiffness() {
		...
		// insert sub-matrices to stiffness matrix
		DMat kLocal = new DMat(78, 78);
		for (int i = 0; i < k1.rowCount(); i++) {
			for (int j = 0; j < k1.columnCount(); j++) {
				kLocal.set(i, j, lambda * k1.get(i, j));
				kLocal.set(i, j + 60, k5.get(i, j));
				kLocal.set(i, j + 72, k4.get(i, j));
				kLocal.set(i + 6, j + 6, beta * k1.get(i, j));
				kLocal.set(i + 6, j + 60, k6.get(i, j));
				kLocal.set(i + 6, j + 66, -k4.get(i, j));
				kLocal.set(i + 12, j + 12, gamma * k1.get(i, j));
				kLocal.set(i + 12, j + 18, alpha * k1.get(i, j));
				kLocal.set(i + 12, j + 48, k2.get(i, j));
				kLocal.set(i + 18, j + 18, phi * k1.get(i, j));
				kLocal.set(i + 18, j + 54, k3.get(i, j));
				kLocal.set(i + 24, j + 24, theta * k1.get(i, j));
				kLocal.set(i + 24, j + 48, k3.get(i, j));
				kLocal.set(i + 24, j + 54, k2.get(i, j));
				kLocal.set(i + 30, j + 30, curlPhi * k1.get(i, j));
				kLocal.set(i + 30, j + 36, capLamb * k1.get(i, j));
				kLocal.set(i + 30, j + 72, k2.get(i, j));
				kLocal.set(i + 36, j + 36, mu * k1.get(i, j));
				kLocal.set(i + 36, j + 66, -k3.get(i, j));
				kLocal.set(i + 42, j + 42, kappa * k1.get(i, j));
				kLocal.set(i + 42, j + 66, -k2.get(i, j));
				kLocal.set(i + 42, j + 72, k3.get(i, j));
			}
		}
	...
	private DMat computeGop(double eps1, double eps2, DMat jac) {
		...
		DMat gop = new DMat(4, 30);

		// loop over nodes of element
		for (int i = 0; i < 6; i++) {

			// compute nodal G operator
			DMat gopNode = new DMat(4, 5);
