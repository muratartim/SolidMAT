...
/**
 * Class for Element13. Properties of element are; Mechanics: Timoshenko beam,
 ...
 * M3. Note: Transverse shear terms are under-integrated against shear locking.
 * 
 * @author Murat
 * 
 */
public class Element13 extends Element1D {
	 ...
	 */
	public Element13(Node n1, Node n2) {
		nodes_[0] = n1;
		nodes_[1] = n2;
	}
	...
	public DMat getStrain(double eps1, double eps2, double eps3) {
		...
		// compute mean values of rotations through element
		double rot2 = 0.0, rot3 = 0.0;
		for (int i = 0; i < nodes_.length; i++) {
			rot2 += 0.5 * uLocal.get(6 * i + 4);
			rot3 += 0.5 * uLocal.get(6 * i + 5);
		}
		...
		for (int i = 0; i < nodes_.length; i++) {
			...
			// compute strain-11 (computed at center of axis -> x3 = 0)
			...
			strain[0][1] += uLocal.get(6 * i + 1)
					...
					* rot3 * shapeF.getFunction(eps1, i);
	...
	public double getInternalForce(int type, double eps1, double eps2,
		...
		// shear force V2 demanded
			...
			// compute mean value of rotations through element
			double rot = 0.0;
			for (int i = 0; i < nodes_.length; i++)
				rot += 0.5 * uLocal.get(6 * i + 5);
			...
			for (int i = 0; i < nodes_.length; i++)
				force += scale
						* (uLocal.get(6 * i + 1)
								* shapeF.getDerFunction(eps1, i) / jac - rot
	...
	protected DMat computeStiffnessMatrix() {
		...
		for (int i = 0; i < k1.rowCount(); i++) {
			for (int j = 0; j < k1.columnCount(); j++) {
				...
				kLocal.set(i, j + 14, k2.get(i, j));
				kLocal.set(i, j + 22, -k3.get(i, j));
				kLocal.set(i + 2, j + 2, beta * k1.get(i, j));
				kLocal.set(i + 2, j + 16, k2.get(i, j));
				kLocal.set(i + 2, j + 20, k3.get(i, j));
				kLocal.set(i + 4, j + 4, gamma * k1.get(i, j));
				kLocal.set(i + 4, j + 12, k2.get(i, j));
				kLocal.set(i + 6, j + 6, alpha * k1.get(i, j));
				kLocal.set(i + 6, j + 22, k2.get(i, j));
				kLocal.set(i + 8, j + 8, phi * k1.get(i, j));
				kLocal.set(i + 8, j + 20, k2.get(i, j));
				kLocal.set(i + 10, j + 10, theta * k1.get(i, j));
				kLocal.set(i + 10, j + 18, k2.get(i, j));
			}
		}
		kLocal = kLocal.mirror();

		// condense out force degrees of freedom
