...
/**
 * Class for Element27. Properties of element are; Mechanics: XZ Planar Curved
 ...
 * 
 */
public class Element27 extends Element1D {
	 ...
	 */
	public Element27(Node n1, Node n2) {
		nodes_[0] = n1;
		nodes_[1] = n2;
	}
	...
	public DMat getStrain(double eps1, double eps2, double eps3) {
		...
		double r1 = getParameters()[0];

		// compute mean values of displacements/rotations through element
		double rot2 = 0.0, disp1 = 0.0, disp3 = 0.0;
		for (int i = 0; i < nodes_.length; i++) {
			disp1 += 0.5 * uLocal.get(3 * i);
			disp3 += 0.5 * uLocal.get(3 * i + 1);
		...
		for (int i = 0; i < nodes_.length; i++) {
			...
			strain[0][0] += uLocal.get(3 * i) * shapeF.getDerFunction(eps1, i)
					/ jacDet + disp3 * shapeF.getFunction(eps1, i) / r1;

			// compute strain-13
			strain[0][2] += uLocal.get(3 * i + 1)
					* shapeF.getDerFunction(eps1, i) / (2.0 * jacDet) + 0.5
					* rot2 * shapeF.getFunction(eps1, i) - disp1
					* shapeF.getFunction(eps1, i) / (2.0 * r1);
		}
	...
	public double getInternalForce(int type, double eps1, double eps2,
		...
		if (type == Element1D.N1_) {
			...
			double scale1 = a / (s11 * rad1);
			for (int i = 0; i < nodes_.length; i++)
				force += scale * uLocal.get(3 * i)
						* shapeF.getDerFunction(eps1, i) + scale1 * disp3
		...
		// shear force V3 demanded
			...
			// compute internal force
			double scale = a2 / s55;
			double scale1 = a2 / (s55 * rad1);
			for (int i = 0; i < nodes_.length; i++)
				force += scale
						* (uLocal.get(3 * i + 1)
								* shapeF.getDerFunction(eps1, i) / jac + rot2
								* shapeF.getFunction(eps1, i)) - scale1 * disp1
						* shapeF.getFunction(eps1, i);
		}
