...
/**
 * Class for Element7. Properties of element are; Mechanics: Plane (plane
 * stress), Geometric: Linear, Material: Orthotropic/Isotropic, Interpolation
 * degree: Bicubic, Interpolation family: Serendipity, Number of nodes: 12,
 * Geometry: Quadrilateral, Dofs (per node): ux, uy, uz (global), u1, u2
 * (local), Mechanical loading: fx, fy, fz (global), f1, f2 (local), Temperature
 * loading: yes, Displacements: u1, u2, Strains: e11, e22, e12, e33, Stresses:
 * s11, s22, s12, Internal forces: P11, N22, Q12.
 * 
 * @author Murat
 * 
 */
public class Element7 extends Element2D {

	private static final long serialVersionUID = 1L;

	/** The nodes of element. */
	private Node[] nodes_ = new Node[12];
	...
	/**
	 * Creates Element7 element. Nodes should be given in counter-clockwise
	 * order.
	 * 
	 * @param n1
	 ...
	 * @param n5
	 *            The fifth node of element (bottom-middle-left).
	 * @param n6
	 *            The sixth node of element (bottom-middle-right).
	 * @param n7
	 *            The seventh node of element (right-middle-down).
	 * @param n8
	 *            The eighth node of element (right-middle-up).
	 * @param n9
	 *            The ninth node of element (top-middle-right).
	 * @param n10
	 *            The tenth node of element (top-middle-left).
	 * @param n11
	 *            The eleventh node of element (left-middle-up).
	 * @param n12
	 *            The twelvth node of element (left-middle-down).
	 */
	public Element7(Node n1, Node n2, Node n3, Node n4, Node n5, Node n6,
			Node n7, Node n8, Node n9, Node n10, Node n11, Node n12) {
		nodes_[0] = n1;
		nodes_[1] = n2;
		nodes_[2] = n3;
		nodes_[3] = n4;
	...
	public DMat getStrain(double eps1, double eps2, double eps3) {
		...
		DMat c = m.getC(Material.planeStress_);
		DMat s = m.getS(Material.threeD_);
		DMat s1 = new DMat(1, 3);
		s1.set(0, 0, s.get(2, 0));
		s1.set(0, 1, s.get(2, 1));
		s1.set(0, 2, s.get(2, 5));
		...
		// compute strain-33
		double e33 = s1.multiply(c.multiply(pStrain.subtract(alpha))).get(0);
	...
	protected Interpolation2D getInterpolation() {
		...
		int degree = Interpolation2D.bicubic_;
	...
	protected DMat computeStiffnessMatrix() {
		...
		// get factors
		DMat c = getMaterial().getC(Material.planeStress_);
		double h = getSection().getDimension(Section.thickness_);
	...
	protected DMat computeMassMatrix() {
		...
		for (int i = 0; i < nog; i++) {
			...
			for (int j = 0; j < nog; j++) {
				...
				func = func.scale(alpha1 * alpha2 * ro * h * jacDet);
	...
	private void tieInternalNodes() {
		...
		nodes_[6].setTransformation(tr);
		nodes_[7].setTransformation(tr);
		nodes_[8].setTransformation(tr);
		nodes_[9].setTransformation(tr);
		nodes_[10].setTransformation(tr);
		nodes_[11].setTransformation(tr);
	}
}
