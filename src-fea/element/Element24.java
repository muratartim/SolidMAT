...
/**
 * Class for Element24. Properties of element are; Mechanics: Mindlin/Reissner
 ...
 * u3, r1, r2 (local), Mechanical loading: fx, fy, fz, mx, my, mz (global), f3,
 * m1, m2 (local), Temperature loading: no, Displacements: u3, r1, r2, Strains:
 * e11, e22, e12, e13, e23 Stresses: s11, s22, s33, s12, s13, s23, Internal
 * forces: F13, H23, K22, M11, T12. Note: Transverse shear terms are
 * under-integrated against shear locking.
 * 
 * @author Murat
 * 
 */
public class Element24 extends Element2D {
	 ...
	 */
	public Element24(Node n1, Node n2, Node n3, Node n4) {
		nodes_[0] = n1;
		nodes_[1] = n2;
		nodes_[2] = n3;
		nodes_[3] = n4;
	...
	protected DMat computeMassMatrix() {
		...
		for (int i = 0; i < k1.rowCount(); i++) {
			for (int j = 0; j < k1.columnCount(); j++) {
				...
				mLocal.set(i + 4, j + 4, i2 * k1.get(i, j));
				mLocal.set(i + 8, j + 8, i2 * k1.get(i, j));
			}
		}
	...
	private DMat getUncondensedStiffness() {
		...
		DMat kLocal = new DMat(32, 32);
		for (int i = 0; i < k1.rowCount(); i++) {
			for (int j = 0; j < k1.columnCount(); j++) {
				kLocal.set(i, j, lambda * k1.get(i, j));
				kLocal.set(i, j + 20, k5.get(i, j));
				kLocal.set(i, j + 28, k4.get(i, j));
				kLocal.set(i + 4, j + 4, beta * k1.get(i, j));
				kLocal.set(i + 4, j + 20, k6.get(i, j));
				kLocal.set(i + 4, j + 24, -k4.get(i, j));
				kLocal.set(i + 8, j + 8, curlPhi * k1.get(i, j));
				kLocal.set(i + 8, j + 12, capLamb * k1.get(i, j));
				kLocal.set(i + 8, j + 28, k2.get(i, j));
				kLocal.set(i + 12, j + 12, mu * k1.get(i, j));
				kLocal.set(i + 12, j + 24, -k3.get(i, j));
				kLocal.set(i + 16, j + 16, kappa * k1.get(i, j));
				kLocal.set(i + 16, j + 24, -k2.get(i, j));
				kLocal.set(i + 16, j + 28, k3.get(i, j));
			}
		}
