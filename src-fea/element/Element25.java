...
/**
 * Class for Element25. Properties of element are; Mechanics: Mindlin/Reissner
 * plate, Geometric: Linear, Material: Orthotropic/Isotropic, Interpolation
 * degree: Biquadratic, Interpolation family: Serendipity, Number of nodes: 8,
 ...
 * 
 */
public class Element25 extends Element2D {
	 ...
	 */
	public Element25(Node n1, Node n2, Node n3, Node n4, Node n5, Node n6,
			Node n7, Node n8) {
		nodes_[0] = n1;
		nodes_[1] = n2;
		nodes_[2] = n3;
	...
	protected DMat computeStiffnessMatrix() {
		...
		for (int i = 0; i < 24; i++)
			for (int j = 0; j < 3; j++)
				for (int k = 0; k < 8; k++)
					kLocal1.set(i, j + 3 * k, kLocal.get(i, 8 * j + k));
		...
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 24; j++)
				for (int k = 0; k < 8; k++)
					kLocal2.set(i + 3 * k, j, kLocal1.get(8 * i + k, j));
	...
	protected DMat computeMassMatrix() {
		...
		for (int i = 0; i < 24; i++)
			for (int j = 0; j < 3; j++)
				for (int k = 0; k < 8; k++)
					mLocal1.set(i, j + 3 * k, mLocal.get(i, 8 * j + k));
		...
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 24; j++)
				for (int k = 0; k < 8; k++)
					mLocal2.set(i + 3 * k, j, mLocal1.get(8 * i + k, j));
	...
	private DMat getUncondensedStiffness() {
		...
		DMat kLocal = new DMat(64, 64);
		for (int i = 0; i < k1.rowCount(); i++) {
			for (int j = 0; j < k1.columnCount(); j++) {
				kLocal.set(i, j, lambda * k1.get(i, j));
				kLocal.set(i, j + 40, k5.get(i, j));
				kLocal.set(i, j + 56, k4.get(i, j));
				kLocal.set(i + 8, j + 8, beta * k1.get(i, j));
				kLocal.set(i + 8, j + 40, k6.get(i, j));
				kLocal.set(i + 8, j + 48, -k4.get(i, j));
				kLocal.set(i + 16, j + 16, curlPhi * k1.get(i, j));
				kLocal.set(i + 16, j + 24, capLamb * k1.get(i, j));
				kLocal.set(i + 16, j + 56, k2.get(i, j));
				kLocal.set(i + 24, j + 24, mu * k1.get(i, j));
				kLocal.set(i + 24, j + 48, -k3.get(i, j));
				kLocal.set(i + 32, j + 32, kappa * k1.get(i, j));
				kLocal.set(i + 32, j + 48, -k2.get(i, j));
				kLocal.set(i + 32, j + 56, k3.get(i, j));
			}
		}
