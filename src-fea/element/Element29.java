...
/**
 * Class for Element29. Properties of element are; Mechanics: XZ Planar Curved
 ...
 * forces: N1, V3, M2. Note: Element must be used in XZ plane.
 * 
 * @author Murat
 * 
 */
public class Element29 extends Element1D {
	 ...
	 */
	public Element29(Node n1, Node n2, Node n3, Node n4) {
		...
		if (type == Element1D.N1_) {
			...
			for (int i = 0; i < nodes_.length; i++)
				force += scale * uLocal.get(3 * i)
						* shapeF.getDerFunction(eps1, i) + scale1
						* uLocal.get(3 * i + 1) * shapeF.getFunction(eps1, i);
				...
				force += scale
						* (uLocal.get(3 * i + 1)
								* shapeF.getDerFunction(eps1, i) / jac + uLocal
								.get(3 * i + 2)
								* shapeF.getFunction(eps1, i)) - scale1
						* uLocal.get(3 * i) * shapeF.getFunction(eps1, i);
		}
	...
	protected DMat computeStiffnessMatrix() {
		...
		for (int i = 0; i < k1.rowCount(); i++) {
			for (int j = 0; j < k1.columnCount(); j++) {
				...
				kLocal.set(i, j + 12, -kappa * k1.get(i, j));
				...
				kLocal.set(i + 4, j + 16, kappa * k1.get(i, j));
