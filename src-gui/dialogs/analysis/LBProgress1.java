...
import analysis.LinearBuckling;

/**
 * Class for Linear Buckling Analysis Progress monitoring.
 * 
 * @author Murat
 * 
 */
public class LBProgress1 extends JDialog implements ActionListener {
	...
	private LinearBuckling analysis_;
	 ...
	 */
	public LBProgress1(RunAnalysis1 owner, Analysis analysis) {

		// build dialog, determine owner dialog, give caption, make it modal
		super(owner, "Linear Buckling Analysis", true);
		analysis_ = (LinearBuckling) analysis;
		owner_ = owner;
		...
		JLabel label4 = new JLabel("Number of modes found :");
		JLabel label5 = new JLabel("Minimum load factor :");
		label6_ = new JLabel(" ");
		label6_.setPreferredSize(new Dimension(300, 14));
		...
		Commons.addComponent(panel3, label6_, 5, 0, 2, 1);
		Commons.addComponent(panel3, progressBar1_, 6, 0, 2, 1);
		...
		timer1_ = new Timer(ONE_SECOND, new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				// set message
				label6_.setText(analysis_.getStatus());
				...
				if (analysis_.isKilled() || analysis_.isCompleted()) {
					...
					if (analysis_.isCompleted()) {
						...
						textfield5_.setText(owner_.owner_.formatter_
								.format(analysis_.getEigenvalues()[0]));
					}
				}
			}
		});
	...
	public void actionPerformed(ActionEvent e) {
		...
		// kill button clicked
			...
			// set text to label
			label6_.setText("Analysis interrupted by user.");
		}
