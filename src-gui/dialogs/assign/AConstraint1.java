...
/**
 * Class for Assign Constraints Assign menu.
 * 
 * @author Murat
 * 
 */
public class AConstraint1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public AConstraint1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Constraints", true);
		owner_ = owner;
		...
		radiobutton4_ = new JRadioButton("Replace existing constraints", true);
		radiobutton5_ = new JRadioButton("Delete existing constraints", false);
	...
	private void initialize() {
		...
		Vector<Constraint> object = owner_.inputData_.getConstraint();
	...
	private void actionOk() {
		...
		if (checkSelection()) {
			...
			if (radiobutton4_.isSelected()) {
				...
				c = owner_.inputData_.getConstraint().get(index);
			}
			...
			if (checkIds()) {
				...
				for (int i = 0; i < nodeIds_.size(); i++)
					owner_.structure_.getNode(nodeIds_.get(i)).setConstraint(c);
	...
	private boolean checkIds() {
		...
		try {
			...
			// display message
			JOptionPane.showMessageDialog(AConstraint1.this,
