...
/**
 * Class for Assign Line Sections Assign menu.
 * 
 * @author Murat
 * 
 */
public class ALineSection1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public ALineSection1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Sections", true);
		owner_ = owner;
		...
		radiobutton4_ = new JRadioButton("Replace existing sections", true);
		radiobutton5_ = new JRadioButton("Delete existing sections", false);
	...
	private void initialize() {
		...
		Vector<Section> object = owner_.inputData_.getSection();
		for (int i = 0; i < object.size(); i++) {
			int type = object.get(i).getType();
	...
	private void actionOk() {
		...
		if (checkSelection()) {
			...
			if (radiobutton4_.isSelected()) {
				...
				m = owner_.inputData_.getSection().get(index);
			}

			// check elements
			if (checkIds()) {

				// set section to lines
	...
	private boolean setIdsFromText() {
		...
		try {
			...
			for (int i = 0; i < comp.length; i++) {
				...
				// check if negative
				if (val < 0) {
					JOptionPane.showMessageDialog(ALineSection1.this,
