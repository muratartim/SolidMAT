...
/**
 * Class for Assign Line Mechanical Loads Assign menu.
 * 
 * @author Murat
 * 
 */
public class ALineMechLoad1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public ALineMechLoad1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Line Mechanical Loads", true);
		owner_ = owner;
	...
	private void initialize() {
		...
		for (int i = 0; i < object.size(); i++)
			if (object.get(i).getType() == ElementMechLoad.line_)
		...
		if (ml != null) {
			...
			for (int i = 0; i < elementIds_.size(); i++) {
				...
				if (dofv.contains(comp) == false) {
					...
					// display message
					JOptionPane.showMessageDialog(ALineMechLoad1.this,
