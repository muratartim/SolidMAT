...
/**
 * Class for Assign Nodal Displacement Loads Assign menu.
 * 
 * @author Murat
 * 
 */
public class ANodalDispLoad1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public ANodalDispLoad1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Nodal Displacements", true);
		owner_ = owner;
	...
	private void actionOk() {
		...
		if (checkSelection()) {
			...
			if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
				...
				dl = owner_.inputData_.getDispLoad().get(index);
			}
			...
			if (checkIds()) {
				...
				for (int i = 0; i < nodeIds_.size(); i++) {
					...
					Vector<DispLoad> loads = new Vector<DispLoad>();
	...
	private boolean checkSelection() {
		...
		if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
			if (list1_.isSelectionEmpty()) {
				...
				// display message
				JOptionPane.showMessageDialog(ANodalDispLoad1.this,
