...
/**
 * Class for Assign Solid Mechanical Loads Assign menu.
 * 
 * @author Murat
 * 
 */
public class ASolidMechLoad1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public ASolidMechLoad1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Solid Mechanical Loads", true);
		owner_ = owner;
	...
	private void initialize() {
		...
		for (int i = 0; i < object.size(); i++)
			if (object.get(i).getType() == ElementMechLoad.volume_)
		...
		if (ml != null) {
			...
			for (int i = 0; i < elementIds_.size(); i++) {
				...
				if (dofv.contains(comp) == false) {
					...
					// display message
					JOptionPane.showMessageDialog(ASolidMechLoad1.this,
