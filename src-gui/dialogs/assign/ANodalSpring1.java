...
/**
 * Class for Assign Nodal Springs Assign menu.
 * 
 * @author Murat
 * 
 */
public class ANodalSpring1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public ANodalSpring1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Nodal Springs", true);
		owner_ = owner;
	...
	private void initialize() {
		...
		Vector<NodalSpring> object = owner_.inputData_.getNodalSpring();
	...
	private void actionOk() {
		...
		if (checkSelection()) {
			...
			if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
				...
				s = owner_.inputData_.getNodalSpring().get(index);
			}
			...
			if (checkIds()) {
				...
				for (int i = 0; i < nodeIds_.size(); i++) {
					...
					// create vector for nodal springs
					Vector<NodalSpring> springs = new Vector<NodalSpring>();
	...
	private boolean checkSelection() {
		...
		if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
			if (list1_.isSelectionEmpty()) {
				...
				// display message
				JOptionPane.showMessageDialog(ANodalSpring1.this,
