...
/**
 * Class for Assign Line Local Axes Assign menu.
 * 
 * @author Murat
 * 
 */
public class ALineLocalAxis1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public ALineLocalAxis1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Line Local Axes", true);
		owner_ = owner;
	...
	private void initialize() {
		...
		for (int i = 0; i < object.size(); i++)
			if (object.get(i).getType() == LocalAxis.line_)
	...
	private void actionOk() {
		...
		if (checkSelection()) {
			...
			if (checkIds()) {
				...
				// set local axis to elements
				for (int i = 0; i < elementIds_.size(); i++) {

					// get one dimensional element
					Element1D e1D = (Element1D) owner_.structure_
	...
	private boolean setIdsFromText() {
		...
		try {
			...
			for (int i = 0; i < comp.length; i++) {
				...
				// check if negative
				if (val < 0) {
					JOptionPane.showMessageDialog(ALineLocalAxis1.this,
