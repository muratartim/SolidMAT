...
/**
 * Class for Assign Initial Velocities Assign menu.
 * 
 * @author Murat
 * 
 */
public class AInitialVelo1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public AInitialVelo1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Initial Velocities", true);
		owner_ = owner;
	...
	private void actionOk() {
		...
		if (checkSelection()) {
			...
			if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
				...
				ml = owner_.inputData_.getInitialVelo().get(index);
			}
			...
			if (checkIds()) {
				...
				for (int i = 0; i < nodeIds_.size(); i++) {
					...
					Vector<InitialVelo> loads = new Vector<InitialVelo>();
	...
	private boolean checkSelection() {
		...
		if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
			if (list1_.isSelectionEmpty()) {
				...
				// display message
				JOptionPane.showMessageDialog(this, "No load selected!",
						"False data entry", 2);
				return false;
			}
		}
