...
/**
 * Class for Assign Nodal Masses Assign menu.
 * 
 * @author Murat
 * 
 */
public class ANodalMass1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public ANodalMass1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Nodal Masses", true);
		owner_ = owner;
	...
	private void initialize() {
		...
		Vector<NodalMass> object = owner_.inputData_.getNodalMass();
	...
	private void actionOk() {
		...
		if (checkSelection()) {
			...
			if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
				...
				m = owner_.inputData_.getNodalMass().get(index);
			}
			...
			if (checkIds()) {
				...
				for (int i = 0; i < nodeIds_.size(); i++) {
					...
					// create vector for nodal masses
					Vector<NodalMass> masses = new Vector<NodalMass>();
	...
	private boolean checkSelection() {
		...
		if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
			if (list1_.isSelectionEmpty()) {
				...
				// display message
				JOptionPane.showMessageDialog(ANodalMass1.this,
