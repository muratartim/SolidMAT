...
/**
 * Class for Assign Solid Materials Assign menu.
 * 
 * @author Murat
 * 
 */
public class ASolidMaterial1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public ASolidMaterial1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Materials", true);
		owner_ = owner;
		...
		radiobutton4_ = new JRadioButton("Replace existing materials", true);
		radiobutton5_ = new JRadioButton("Delete existing materials", false);
		...
		Commons.addComponent(panel5, radiobutton4_, 0, 0, 1, 1);
		Commons.addComponent(panel5, radiobutton5_, 1, 0, 1, 1);
	...
	private void initialize() {
		...
		Vector<Material> object = owner_.inputData_.getMaterial();
	...
	private void actionOk() {
		...
		// check if any material selected
		if (checkSelection()) {
			...
			if (radiobutton4_.isSelected()) {
				...
				m = owner_.inputData_.getMaterial().get(index);
			}

			// check elements
			if (checkIds()) {

				// set material to solids
	...
	private boolean checkSelection() {
		...
		if (radiobutton4_.isSelected()) {
			if (list1_.isSelectionEmpty()) {
				...
				// display message
				JOptionPane.showMessageDialog(this, "No material selected!",
						"False data entry", 2);
				return false;
			}
		}
	...
	private boolean setIdsFromText() {
		...
		try {
			...
			for (int i = 0; i < comp.length; i++) {
				...
				// check if negative
				if (val < 0) {
					JOptionPane.showMessageDialog(ASolidMaterial1.this,
