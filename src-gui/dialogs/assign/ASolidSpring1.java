...
/**
 * Class for Assign Solid Springs Assign menu.
 * 
 * @author Murat
 * 
 */
public class ASolidSpring1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public ASolidSpring1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Solid Springs", true);
		owner_ = owner;
		...
		radiobutton3_ = new JRadioButton("Add to existing springs", false);
		radiobutton4_ = new JRadioButton("Replace existing springs", true);
		radiobutton5_ = new JRadioButton("Delete existing springs", false);
	...
	private void initialize() {
		...
		Vector<ElementSpring> object = owner_.inputData_.getElementSpring();
			...
			if (checkIds()) {
				...
				if (checkSpring(s)) {
					...
					for (int i = 0; i < elementIds_.size(); i++) {
						...
						// create vector for element springs
						Vector<ElementSpring> springs = new Vector<ElementSpring>();

						// add option selected
						if (radiobutton3_.isSelected()) {

							// get element's springs
							if (e.getSprings() != null)
								springs = e.getSprings();
	...
	/**
	 * Checks whether given spring is compatible with selected elements.
	 * 
	 * @param s
	 *            The spring to be checked.
	 * @return True if the spring is compatible, False if not.
	 */
	private boolean checkSpring(ElementSpring s) {

		// check if any spring given
		if (s != null) {

			// get components and coordinate system of spring
			int comp = s.getComponent();
			int coord = s.getCoordinateSystem();
			...
			for (int i = 0; i < elementIds_.size(); i++) {
				...
				// check components of spring with element's dofs
				...
				if (dofv.contains(comp) == false) {
					...
					// display message
					JOptionPane.showMessageDialog(ASolidSpring1.this,
							"Illegal spring for element type: " + type,
							"False data entry", 2);
					return false;
				}
			}
	...
	private boolean checkSelection() {
		...
		if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
			if (list1_.isSelectionEmpty()) {
				...
				// display message
				JOptionPane.showMessageDialog(ASolidSpring1.this,
						"No spring selected!", "False data entry", 2);
				return false;
			}
		}
