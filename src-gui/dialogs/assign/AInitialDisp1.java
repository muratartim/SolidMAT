...
/**
 * Class for Assign Initial Displacements Assign menu.
 * 
 * @author Murat
 * 
 */
public class AInitialDisp1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public AInitialDisp1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Initial Displacements", true);
		owner_ = owner;
	...
	private void actionOk() {
		...
		if (checkSelection()) {
			...
			if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
				...
				ml = owner_.inputData_.getInitialDisp().get(index);
			}
			...
			if (checkIds()) {
				...
				for (int i = 0; i < nodeIds_.size(); i++) {
					...
					Vector<InitialDisp> loads = new Vector<InitialDisp>();
	...
	private boolean checkSelection() {
		...
		if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
			if (list1_.isSelectionEmpty()) {
				...
				// display message
				JOptionPane.showMessageDialog(AInitialDisp1.this,
