...
/**
 * Class for Assign Solid Masses Assign menu.
 * 
 * @author Murat
 * 
 */
public class ASolidMass1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public ASolidMass1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Solid Masses", true);
		owner_ = owner;
		...
		radiobutton6_ = new JRadioButton("All existing solids", false);
		radiobutton3_ = new JRadioButton("Add to existing masses", false);
		radiobutton4_ = new JRadioButton("Replace existing masses", true);
		radiobutton5_ = new JRadioButton("Delete existing masses", false);
	...
	private void initialize() {
		...
		Vector<ElementMass> object = owner_.inputData_.getElementMass();
	...
	private void actionOk() {
		...
		if (checkSelection()) {
			...
			if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
				...
				m = owner_.inputData_.getElementMass().get(index);
			}
			...
			if (checkIds()) {
				...
				if (checkMass(m)) {
					...
					for (int i = 0; i < elementIds_.size(); i++) {
						...
						// create vector for element masses
						Vector<ElementMass> masses = new Vector<ElementMass>();

						// add option selected
						if (radiobutton3_.isSelected()) {

							// get element's masses
							if (e.getAdditionalMasses() != null)
								masses = e.getAdditionalMasses();

							// add selected mass to vector
							masses.add(m);
						}
	...
	/**
	 * Checks whether given mass is compatible with selected elements.
	 * 
	 * @param m
	 *            The mass to be checked.
	 * @return True if the mass is compatible, False if not.
	 */
	private boolean checkMass(ElementMass m) {

		// check if any mass given
		if (m != null) {

			// get components and coordinate system of mass
			int comp = m.getComponent();
			int coord = m.getCoordinateSystem();
			...
			for (int i = 0; i < elementIds_.size(); i++) {
				...
				// check components of mass with element's dofs
				...
				if (dofv.contains(comp) == false) {
					...
					// display message
					JOptionPane.showMessageDialog(ASolidMass1.this,
							"Illegal mass for element type: " + type,
							"False data entry", 2);
					return false;
				}
			}
		...
		// mass is compatible with selected elements
		return true;
	}
	...
	private boolean checkSelection() {
		...
		// check if any mass selected
		if (radiobutton3_.isSelected() || radiobutton4_.isSelected()) {
			if (list1_.isSelectionEmpty()) {

				// display message
				JOptionPane.showMessageDialog(ASolidMass1.this,
						"No mass selected!", "False data entry", 2);
				return false;
			}
		}
	...
	private boolean setIdsFromText() {
		...
		try {
			...
			// display message
			JOptionPane.showMessageDialog(ASolidMass1.this,
					"Given solids do not exist!", "False data entry", 2);
			return false;
		}
	...
	private void setIdsFromGroup() {
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			// check if three dimensional
			if (elements.get(i).getDimension() == ElementLibrary.threeDimensional_) {
	...
	private void setIdsForAll() {
		...
		for (int i = 0; i < owner_.structure_.getNumberOfElements(); i++) {
			...
			if (owner_.structure_.getElement(i).getDimension() == ElementLibrary.threeDimensional_)
				elementIds_.add(i);
		}
	}
