...
/**
 * Class for Assign Nodal Local Axes Assign menu.
 * 
 * @author Murat
 * 
 */
public class ANodalLocalAxis1 extends JDialog implements ActionListener,
		ItemListener {
	 ...
	 */
	public ANodalLocalAxis1(SolidMAT owner) {

		// build dialog, determine owner frame, give caption, make it modal
		super(owner.viewer_, "Assign Nodal Local Axes", true);
		owner_ = owner;
		...
		radiobutton4_ = new JRadioButton("Replace existing local axes", true);
		radiobutton5_ = new JRadioButton("Delete existing local axes", false);
	...
	private void initialize() {
		...
		Vector<LocalAxis> object = owner_.inputData_.getLocalAxis();
		for (int i = 0; i < object.size(); i++)
			if (object.get(i).getType() == LocalAxis.point_)
	...
	private void actionOk() {
		...
		if (checkSelection()) {
			...
			if (radiobutton4_.isSelected()) {
				...
				for (int i = 0; i < owner_.inputData_.getLocalAxis().size(); i++) {
					if (name.equals(owner_.inputData_.getLocalAxis().get(i)
							.getName()))
						l = owner_.inputData_.getLocalAxis().get(i);
				}
			}
			...
			if (checkIds()) {
				...
				// set local axis to nodes
				for (int i = 0; i < nodeIds_.size(); i++)
					owner_.structure_.getNode(nodeIds_.get(i)).setLocalAxis(l);
	...
	private boolean checkSelection() {
		...
		if (radiobutton4_.isSelected()) {
			if (list1_.isSelectionEmpty()) {
				...
				// display message
				JOptionPane.showMessageDialog(this, "No local axes selected!",
						"False data entry", 2);
				return false;
			}
		}
	...
	private boolean checkIds() {
		...
		try {
			...
			// display message
			JOptionPane.showMessageDialog(ANodalLocalAxis1.this,
