...
/**
 * Class for Display Nodal Loads menu.
 * 
 * @author Murat
 * 
 */
public class DisplayNodalLoads1 extends JDialog implements ActionListener {
	 ...
	 */
	public DisplayNodalLoads1(SolidMAT owner) {

		// build dialog, determine owner dialog, give caption, make it modal
		super(owner.viewer_, "Show Nodal Loads", true);
		owner_ = owner;
		...
		combobox1_.setPreferredSize(new Dimension(131, 23));

		// build radio buttons and set font
		radiobutton1_ = new JRadioButton("Forces", true);
		radiobutton2_ = new JRadioButton("Displacements", false);
		radiobutton3_ = new JRadioButton("Initial Displacements", false);
		radiobutton4_ = new JRadioButton("Initial Velocities", false);
	...
	private void actionOk() {
		...
		for (int i = 2; i < nodeOpt.length; i++)
			nodeOpt[i] = false;
		nodeOpt[4] = radiobutton1_.isSelected();
		nodeOpt[5] = radiobutton2_.isSelected();
		nodeOpt[8] = radiobutton3_.isSelected();
		nodeOpt[9] = radiobutton4_.isSelected();
