...
/**
 * Class for Linear Buckling Analysis Cases panel.
 * 
 * @author Murat
 * 
 */
public class AnalysisCasePanel4 extends JPanel implements ActionListener,
		FocusListener, ListSelectionListener {

	private static final long serialVersionUID = 1L;

	protected JTextField textfield1_, textfield4_;
	 ...
	 */
	public AnalysisCasePanel4(AnalysisCase2 owner) {
		...
		Commons.addComponent(panel3, textfield4_, 1, 2, 1, 1);
		...
		panel4.add(Box.createRigidArea(new Dimension(0, 229)));
	...
	protected void initialize(Analysis selected) {
		...
		LinearBuckling lb = (LinearBuckling) selected;

		// get solver
		String solver = lb.getSolver().getName();

		// boundaries
		Vector<BoundaryCase> boundaries = lb.getBoundaries();
		double[] scales = lb.getBoundaryScales();
	...
	private void actionAdd() {
		...
		if (listModel1_.contains(boundary) == false) {
			...
			listModel2_.addElement(textfield4_.getText());
		}
	}
	...
	private void actionModify() {
		...
		if (list1_.isSelectionEmpty() == false) {
			...
			if (selected.equals(boundary))
				listModel2_.setElementAt(textfield4_.getText(), index);
	...
	public void focusLost(FocusEvent e) {
		...
		try {
			...
			if (e.getOppositeComponent().equals(null) == false) {
				...
				int messageType = 0;
				if (textfield.equals(textfield4_))
					messageType = 2;
	...
	protected void setDefaultText(JTextField textfield) {
		...
		String defaultValue = owner_.owner_.owner_.formatter_.format(1.0);
